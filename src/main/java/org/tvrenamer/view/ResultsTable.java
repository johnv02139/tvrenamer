...
public final class ResultsTable implements Observer, AddEpisodeListener {
    ...
    private static String getCellStatusString(final TableItem item, final Column column) {
        return ItemState.getImagePriority(item.getImage(column.index));
    }

    private static ItemState.Status getCellStatus(final TableItem item, final Column column) {
        return ItemState.getImageStatus(item.getImage(column.index));
    }
    ...
    {
        item.setImage(column.index, ItemState.getIcon(newStatus));
    }
    ...
    private static void setCellImage(final TableItem item,
                                     ...
                                     final ItemState.Status newStatus)
    {
        item.setImage(Columns.getColumnIndex(columnId), ItemState.getIcon(newStatus));
    }
    ...
    private void setProposedDestColumn(final TableItem item, final FileEpisode ep) {
        ...
        if (nOptions > 1) {
            setComboBoxProposedDest(item, destColumn, ep);
        } else if (nOptions == 1) {
            ...
            switch (column.id) {
                ...
                case STATUS_COLUMN:
                    if (nOptions > 1) {
                        ...
                        FileMoveStatus oldStatus = getCellStatus(item, column);
                        if ((oldStatus == OPTIONS)
                            || (oldStatus == SUCCESS))
                        {
                            setCellImage(item, column, FAIL);
                        }
        ...
        for (Column column : columns) {
            switch (column.id) {
                case CHECKBOX_COLUMN:
                    ...
                    break;
                case STATUS_COLUMN:
                    setCellImage(item, column, DOWNLOADING);
                    break;
        ...
        for (Column column : columns) {
            switch (column.id) {
                case CHECKBOX_COLUMN:
                    ...
                    break;
                case STATUS_COLUMN:
                    setCellImage(item, column, getCellStatus(oldItem, column));
                    break;
                default:
