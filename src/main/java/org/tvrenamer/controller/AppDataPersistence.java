    ...
    public static AppData retrieve(Path path) {
        if (Files.exists(path)) {
            try (InputStream in = Files.newInputStream(path)) {
                ...
                logger.log(Level.SEVERE, "XStream exception reading app data file", xe);
                logger.info("check your file, \"" + path.toAbsolutePath().toString()
                            + "\"; substituting default app data");
            } catch (IOException ioe) {
                logger.log(Level.SEVERE, "I/O exception reading app data file "
                           + path.toAbsolutePath().toString(), ioe);
                logger.info("assuming default app data");
            } catch (Exception e) {
                logger.log(Level.SEVERE, "Unexpected exception reading app data file \""
                           + path.toAbsolutePath().toString() + "\": " + e.getClass(), e);
                logger.info("assuming default app data");
            }
        } else {
        ...
        // By returning null, we're telling the caller to use the defaults
        return null;
    }
}
