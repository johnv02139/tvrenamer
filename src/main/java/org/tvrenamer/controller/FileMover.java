...
public class FileMover implements Callable<Boolean> {
    ...
    public Boolean call() {
        ...
        if (destFile.getParentFile().exists() || destFile.getParentFile().mkdirs()) {
            ResultsTable.setTableItemStatus(display, item, ItemState.RENAMING);
            boolean succeeded = false;
            if (FileUtilities.areSameDisk(srcFile.getAbsolutePath(), destFile.getAbsolutePath())) {
                succeeded = srcFile.renameTo(destFile);
            }
            if (succeeded) {
                ResultsTable.setTableItemStatus(display, item, ItemState.SUCCESS);
                ...
                if (succeeded) {
                    ...
                    ResultsTable.setTableItemStatus(display, item, ItemState.FAIL);
                }
                return succeeded;
            }
        }
